-- ============================================================
-- Sub Category master — create capability + parent category on read
-- Database : Non_Trade (MSSQL, 10.0.21.8)
-- Used by  : backend-stpl/src/Masters (generic /api/common_master/:masterField
--            pipeline), registered as master key "ProductSubCategoryMaster".
--
-- Why this is needed
-- ------------------
-- subcategory_master and sp_product_sub_catagory (the read SP) already
-- existed live, but there was no create procedure and no Masters-grid tile
-- — subcategories could only be selected on a Product, never managed.
-- Table columns (confirmed live): subcat_sno (IDENTITY PK), subcat_name,
-- subcat_description, subcat_notes, subcat_active, subcat_created_date,
-- subcat_created_by, subcat_modified_date, subcat_modified_by, cat_sno
-- (NOT NULL, no DB-level FK constraint — enforced here at the app layer,
-- same as sp_nt_CreateProductRecord already does for its own cat_sno).
--
-- sp_product_sub_catagory is regenerated to also return cat_name (joined
-- from category_master) so the Masters grid can show the parent category by
-- name instead of a raw cat_sno — same pattern product_master's own
-- cat_sno/cat_name pair already uses on the Product form.
-- ============================================================

-- ── sp_product_sub_catagory (GET) — now also returns cat_name ──────────────
...
-- ── sp_nt_CreateSubCategoryRecords ──────────────────────────────────────────
-- @jsonInput: {"cat_sno":1, "subcat_name":"...", "subcat_description":"...",
--              "subcat_notes":"...", "created_by":"..."}
...
--   SELECT * FROM dbo.subcategory_master ORDER BY subcat_sno DESC;
