...
class ServiceVendorEntryController {
  ...
  static async getEntries(req, res) {
    try {
      const { vendor_sno, service_sno, com_sno, div_sno, brn_sno, dept_sno, status, date_from, date_to } = req.query;
      const data = await ServiceVendorEntryService.getEntries({
        vendor_sno, service_sno, com_sno, div_sno, brn_sno, dept_sno, status, date_from, date_to,
      });
      res.json({ success: true, data });
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
